...
namespace CodenameIndigo.Modules
{
    ...
    {
        public static async Task<List<Player>> GenerateRandomBracketsAsync(bool rand = true)
        {
            ...
            try
            {
                ...
                MySqlCommand cmd = new MySqlCommand("SELECT * FROM `participants` WHERE `tid` = " + (await DatabaseHelper.GetLatestTourneyAsync()).ID, conn);
                if (rand)
